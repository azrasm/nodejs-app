let SpisakNekretnina = function () {
    ...
    let filtrirajNekretnine = function (kriterij) {
        return this.listaNekretnina.filter(nekretnina => {
            ...
            if (kriterij.periodObjave && 
            (parseInt(nekretnina.datum_objave.split(".")[2].trim().replace(".", ""))<=kriterij.periodObjave.od ||
            parseInt(nekretnina.datum_objave.split(".")[2].trim().replace(".", ""))>=kriterij.periodObjave.do)) {
                console.log("poredjenje");
                return false;
            }
            if (kriterij.rasponCijene && 
