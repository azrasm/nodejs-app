...
/*
    function impl_postLogin(username, password, fnCallback) {
        ...
        ajax.onreadystatechange = function () {
            if (ajax.readyState == 4 && ajax.status == 200) {
                ...
                //desio se neki error
               // fnCallback({responseText: ajax.responseText }, null);
                fnCallback({ status: ajax.status, responseText: ajax.responseText }, null);
            }
        }
    ...
    function impl_getTop5Nekretnina(lokacija, fnCallback) {
        ...
        ajax.onreadystatechange = function () {
            if (ajax.readyState == 4 && ajax.status == 200) {
                try {
                    ...
                    fnCallback({ greska: 'Neispravan JSON odgovor' }, null);
                }
            }
            else if (ajax.readyState == 4) {
                fnCallback({ status: ajax.status, responseText: ajax.responseText }, null);
        ...
        var url = "http://localhost:3000/nekretnine/top5";
        if (lokacija) {
            url += "?lokacija=" + encodeURIComponent(lokacija);
        }
    ...
    function impl_getNekretnina(id, fnCallback) {
        ...
        var url = `http://localhost:3000/nekretnina/${id}`;
       // var url = `http://localhost:3000/nekretnina/3`;
        console.log(url);
        ajax.open("GET", url, true);
        ajax.send();
    }
    function impl_getMojiUpiti(fnCallback) {
        ...
        var url = `http://localhost:3000/upiti/moji`;
        console.log(url);
        ajax.open("GET", url, true);
        ajax.send();
    }
    ...
    function impl_getNextUpiti(nekretnina_id, page, fnCallback) {
            ...
            if (ajax.readyState === 4) {
                if (ajax.status === 200) {
                    try {
                        var response = JSON.parse(ajax.responseText);
                        console.log("Parsed JSON response:", response);
                        fnCallback(null, response.upiti);
                    } catch (e) {
                        console.error("Error parsing JSON:", e.message);
                        fnCallback({ greska: 'Neispravan JSON odgovor' }, null);
                    }
        ...
        var url = `http://localhost:3000/next/upiti/nekretnina/${encodeURIComponent(nekretnina_id)}?page=${encodeURIComponent(page)}`;
        console.log(`u ruti rute: ${url}`);
